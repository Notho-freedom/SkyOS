...
// const initGroq = () => {
//   try {
//     if (typeof window !== "undefined" && window.Groq) {
//       return new window.Groq({
...
const loadGroqSdk = async () => {
  try {
    // Méthode alternative qui évite le problème de CDN
    const response = await fetch('https://unpkg.com/groq-sdk@latest/dist/umd/index.min.js')
    const scriptText = await response.text()
    const script = document.createElement('script')
    script.text = scriptText
    document.body.appendChild(script)
    
    // Attendre que Groq soit disponible globalement
    await new Promise(resolve => {
      const check = () => {
        if (window.Groq) resolve()
